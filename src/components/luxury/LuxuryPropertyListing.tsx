  ...
  return (
    <a href={`/property/${property.id}`} className="group grid grid-cols-1 md:grid-cols-12 gap-0 bg-neutral-50 border border-neutral-200 rounded-sm overflow-hidden mb-6 hover:shadow-md transition-shadow duration-300">
      ...
      <div className="md:col-span-7 flex flex-col p-5 md:p-6 lg:p-8">
        ...
        <h2 className="text-[17px] md:text-[19px] font-medium text-luxury-black leading-snug mb-1 group-hover:text-luxury-black/75 transition-colors duration-300">
          {property.title}
        </h2>
        <p className="text-[12px] tracking-[0.14em] uppercase text-luxury-black/60 mb-1.5">{property.location}</p>
        <div className="flex items-center gap-3 mb-3">
          <p className="text-[13px] text-luxury-black/55 font-light">Detached houses <span className="mx-1 text-luxury-black/30">|</span> <span className="italic">{property.style}</span></p>
          <span className="text-[11px] font-mono text-luxury-black/40 tracking-wide">REF-{String(property.id).padStart(4, "0")}</span>
        </div>
