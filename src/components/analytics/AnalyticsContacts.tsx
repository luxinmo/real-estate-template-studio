    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <div className="col-span-2 lg:col-span-1 rounded-xl border border-[#C9A96E]/20 bg-gradient-to-br from-[#C9A96E]/5 to-transparent p-5">
          ...
          <p className="text-[36px] font-light text-white tracking-tighter mt-1">{totalContacts}</p>
          <p className="text-[11px] text-[#6a6a7a]">Last 30 days</p>
        </div>
        {contactChannels.map((ch) => (
          <div key={ch.channel} className="rounded-xl border border-[#1a1a2e] bg-[#12121e] p-4">
            <p className="text-[11px] text-[#6a6a7a] mb-2">{ch.channel}</p>
            <p className="text-[22px] font-semibold text-white">{ch.count}</p>
      ...
      <div className="rounded-xl border border-[#1a1a2e] bg-[#12121e] p-5">
        <h3 className="text-[13px] font-semibold text-white mb-4">Contacts Over Time</h3>
        <div className="h-[260px]">
          <ResponsiveContainer>
            <AreaChart data={contactsTimeline}>
              ...
              <Area type="monotone" dataKey="form" stackId="1" stroke="#C9A96E" fill="#C9A96E" fillOpacity={0.3} />
              <Area type="monotone" dataKey="phone" stackId="1" stroke="#8B7355" fill="#8B7355" fillOpacity={0.3} />
              <Area type="monotone" dataKey="email" stackId="1" stroke="#6B8E7B" fill="#6B8E7B" fillOpacity={0.3} />
              <Area type="monotone" dataKey="whatsapp" stackId="1" stroke="#7B9EAE" fill="#7B9EAE" fillOpacity={0.3} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        <div className="flex gap-4 mt-3">
          {[
            ...
            <div key={l.label} className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] text-[#6a6a7a]">{l.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl border border-[#1a1a2e] bg-[#12121e] p-5">
          <h3 className="text-[13px] font-semibold text-white mb-4">Top Contact-Generating Properties</h3>
          <div className="space-y-2">
            {[...topProperties].sort((a, b) => b.contacts - a.contacts).slice(0, 8).map((p, i) => (
              <div key={p.ref} className="flex items-center gap-3">
                ...
                <span className="text-[12px] text-white font-semibold">{p.contacts}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-[#1a1a2e] bg-[#12121e] p-5">
          <h3 className="text-[13px] font-semibold text-white mb-4">Conversion by Country</h3>
          <div className="space-y-2">
            {conversionByCountry.map((c, i) => (
              <div key={c.country} className="flex items-center gap-3">
                <span className="text-sm">{c.flag}</span>
                <span className="text-[12px] text-[#c4c4d4] flex-1">{c.country}</span>
                <span className="text-[12px] text-[#8a8a9a] tabular-nums">{c.contacts} contacts</span>
